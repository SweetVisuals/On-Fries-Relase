...
export const OrderModal: React.FC<OrderModalProps> = ({ isOpen, onClose, orderToEdit }) => {
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {ITEM_ADDONS[customizingItem.name].sauces.map(sauce => {
                    ...
                    const isMainItem = customizingItem.category === 'Main';
                    const isFree = customizingItem.name === 'Kids Meal' && !Object.keys(selectedAddons).some(a => ['Green Sauce'].includes(a) && a !== sauce);
                    if (isMainItem) {
                      return (
      ...
      {!isOrderDetailsCollapsed && (
        <div className="p-4 pt-0">
          <div className="p-4 border-b border-zinc-800 bg-zinc-900/50 space-y-3">
            ...
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Special Instructions / Notes" className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-brand-yellow focus:outline-none font-medium resize-none h-20 text-sm" />
          </div>
  ...
  const CartDetails = () => (
    <div className="flex flex-col h-full bg-zinc-900">
      ...
      <div className="p-4 border-b border-zinc-800 bg-zinc-900/50 space-y-3">
        <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="Customer Name / Table No." className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-brand-yellow focus:outline-none font-medium" autoFocus disabled={!!orderToEdit} />
        <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Special Instructions / Notes" className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-brand-yellow focus:outline-none font-medium resize-none h-24 text-sm" />
      </div>
      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-zinc-900">
        {cart.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-zinc-500 space-y-2 opacity-50">
