    ...
    <AdminLayout>
      <div className="flex flex-col space-y-8 max-w-7xl mx-auto animate-fade-in">
        ...
        <div className="flex justify-between items-center py-4 border-b border-zinc-800">
          <div>
            <h2 className="text-4xl font-bold text-white font-heading uppercase">Kitchen Display</h2>
            <p className="text-zinc-400 mt-1 font-sans">Manage active orders</p>
          </div>
          <button
            onClick={handleNewOrder}
            className="bg-brand-yellow hover:bg-yellow-400 text-black font-bold px-6 py-3 rounded-xl flex items-center gap-2 transition-all shadow-lg shadow-yellow-500/20"
          >
        ...
        <div>
          <div className="flex items-center gap-2 mb-4 text-zinc-400 uppercase text-xs font-bold tracking-wider">
            <Flame className="w-4 h-4 text-orange-500" /> Grill Station Summary
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl flex justify-between items-center">
              <div className="flex flex-col">
                <Beef className="w-6 h-6 text-zinc-500 mb-2" />
                <span className="text-sm font-bold text-white">Steaks</span>
                <span className="text-xs text-zinc-500">to cook</span>
              </div>
              <div className="text-3xl font-bold text-white">{grillStats.steaks}</div>
            </div>
            <div className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl flex justify-between items-center">
              <div className="flex flex-col">
                <Beef className="w-6 h-6 text-zinc-500 mb-2" />
                <span className="text-sm font-bold text-white">Lamb</span>
                <span className="text-xs text-zinc-500">to cook</span>
              </div>
              <div className="text-3xl font-bold text-white">{grillStats.lamb}</div>
            </div>
            <div className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl flex justify-between items-center">
              <div className="flex flex-col">
                <Flame className="w-6 h-6 text-zinc-500 mb-2" />
                <span className="text-sm font-bold text-white">Short Rib</span>
                <span className="text-xs text-zinc-500">to cook</span>
              </div>
              <div className="text-3xl font-bold text-white">{grillStats.shortRib}</div>
            </div>
            <div className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl flex justify-between items-center">
              <div className="flex flex-col">
                <Carrot className="w-6 h-6 text-zinc-500 mb-2" />
                <span className="text-sm font-bold text-white">Kids Cone</span>
                <span className="text-xs text-zinc-500">to cook</span>
              </div>
              <div className="text-3xl font-bold text-white">{grillStats.kidsCone}</div>
            </div>
          </div>
        </div>

        {/* Stats Row */}
        <div className="flex gap-4 overflow-x-auto pb-2 border-t border-zinc-800 pt-8">
          {/* Pending Card Removed as per request */}
          <div className="px-6 py-3 bg-zinc-900 border border-zinc-800 rounded-xl flex items-center gap-3 min-w-[200px]">
            <div className="p-2 bg-orange-500/10 rounded-lg"><ChefHat className="w-5 h-5 text-orange-500" /></div>
            <div>
              <div className="text-2xl font-bold text-white">{activeOrders.filter(o => o.status === 'cooking').length}</div>
              <div className="text-xs text-zinc-500 font-medium uppercase">Cooking</div>
            </div>
          </div>
          <div className="px-6 py-3 bg-zinc-900 border border-zinc-800 rounded-xl flex items-center gap-3 min-w-[200px]">
            <div className="p-2 bg-emerald-500/10 rounded-lg"><CheckCircle className="w-5 h-5 text-emerald-500" /></div>
            <div>
              <div className="text-2xl font-bold text-white">{activeOrders.filter(o => o.status === 'ready').length}</div>
              <div className="text-xs text-zinc-500 font-medium uppercase">Ready to Serve</div>
            </div>
          </div>
        </div>
        ...
        {activeOrders.length === 0 ? (
          <div className="text-center py-20 bg-zinc-900/30 border-2 border-dashed border-zinc-800 rounded-2xl">
            ...
            <p className="text-zinc-500 mt-2">No active orders in the kitchen.</p>
          </div>
        ) : (
              ...
              <div key={order.id} className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden flex flex-col shadow-xl hover:border-zinc-700 transition-all">
                ...
                <div className="p-5 border-b border-zinc-800 flex justify-between items-start bg-zinc-800/30">
                  <div>
                    ...
                    <h3 className="font-medium text-zinc-300">{order.customerName}</h3>
                  </div>
                  <div className="flex gap-1">
                    <button onClick={() => handleEdit(order)} className="p-2 text-zinc-500 hover:text-white hover:bg-zinc-800 rounded-lg transition-colors"><Edit className="w-4 h-4" /></button>
                    <button onClick={() => handleDeleteOrder(order.id)} className="p-2 text-zinc-500 hover:text-red-400 hover:bg-zinc-800 rounded-lg transition-colors"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>

                {/* Timer Info */}
                <div className="px-5 py-3 bg-zinc-950/30 flex items-center gap-2 text-xs font-medium text-zinc-500 border-b border-zinc-800/50">
                    ...
                    <div key={idx} className="flex justify-between items-start">
                      <div className="flex-1">
                        <div className="flex items-baseline gap-2">
                          <span className="font-bold text-brand-yellow text-lg">{item.quantity}x</span>
                          <span className="font-medium text-white">{item.name}</span>
                        </div>
                        {item.addons && item.addons.length > 0 && (
                          <div className="mt-1 pl-6 space-y-1">
                            {item.addons.map(addon => (
                              <div key={addon} className="text-xs text-zinc-400 flex items-center gap-1">
                ...
                <div className="p-4 border-t border-zinc-800 bg-zinc-900 mt-auto">
                  <button
                    ...
                    className={`w-full py-3.5 rounded-xl font-bold text-sm transition-all flex justify-center items-center gap-2 shadow-lg ${order.status === 'pending' ? 'bg-brand-yellow text-black hover:bg-yellow-400 shadow-yellow-900/20' :
