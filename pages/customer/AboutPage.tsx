    ...
    <CustomerLayout>
      <div className="space-y-8 animate-fade-in max-w-3xl mx-auto pb-8">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden">
           ...
           <div className="p-6 grid gap-4">
              {getDisplayHours().map((schedule, index) => (
                <div key={schedule.days} className={`flex justify-between items-center ${index < getDisplayHours().length - 1 ? 'border-b border-zinc-800 pb-3' : ''}`}>
                   <span className="text-zinc-400">{schedule.days}</span>
                   <span className={`font-bold ${schedule.days.includes('Fri') || schedule.days.includes('Sat') ? 'text-brand-yellow' : 'text-white'}`}>{schedule.hours}</span>
                </div>
              ))}
           </div>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden">
           ...
           <div className="h-64 w-full bg-zinc-800 relative cursor-pointer" onClick={() => window.open('https://maps.google.com/maps?q=Clock%20Tower%2C%20Kings%20Shade%20Walk%2C%20Epsom%20KT19%208EB', '_blank')}>
              <iframe
